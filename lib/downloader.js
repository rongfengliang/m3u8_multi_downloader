...
function download(opts, cb) {
    ...
    var  options = {
        url: url,
        headers: {
          'User-Agent': 'Mozilla/5.0 (Windows NT 6.0) AppleWebKit/537.11 (KHTML, like Gecko) Chrome/111.0.1271.97 Safari/537.11',
          'Referer':referer
        }
      };
    request(options, (err, res, body) => {
        ...
        parseM3U8(body,referer);
    });
}

function parseM3U8(content,referer) {
...
function batchDownload(referer) {
    for (let i = 0; i < processNum; i++) {
        downloadTs(i,referer);
    }
}

function decryptTs(index, tsObj, needLog,referer) {
    if (!fs.existsSync(`${tsObj.file}x`)) {
        ...
        exec(cmd, (_error) => {
            ...
            checkIfDone();
            downloadTs(index + processNum,referer);
        });
    } else {
        downloadedNum++;
        utils.log(`openssl ts ${index} success`);
...
function downloadTs(index,referer) {
    if (index >= tsCount) {
        return;
    }
            ...
            if (error) {
                ...
                fs.writeFile(tsObj.file, buff, (error2) => {
                    if (error2) {
                        ...
                        decryptTs(index, tsObj, true,referer);
                    }
                });
            }
        });
